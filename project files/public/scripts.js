...
// Get Categories with Lost Items over Threshold
async function fetchCategoriesWithLostItemsOver(event) {
    event.preventDefault();

    const threshold = document.getElementById('lostItemsThreshold').value;
    const response = await fetch(`/stats/categories-with-lost-items-over/${threshold}`, { method: 'GET' });
    const responseData = await response.json();
    const tableBody = document.getElementById('categoriesWithLostItemsTableBody');
    tableBody.innerHTML = '';
...
// Get Users Reporting All Categories
async function fetchUsersReportingAllCategories() {
    const response = await fetch('/stats/users-reporting-all-categories', { method: 'GET' });
    const responseData = await response.json();
    const tableBody = document.getElementById('usersReportingAllCategoriesTableBody');
    tableBody.innerHTML = '';
